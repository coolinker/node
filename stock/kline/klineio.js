...
function getSZDayInc(date) {
    var obj = szDateObj[date];
    return
}
...
function readKLineBaseInDateMap(stockId) {
    var kLineJson = this.readKLineBaseSync(stockId);
    var dateObj = {};
    for (var i = 0; i < kLineJson.length; i++) {
        var obj = kLineJson[i];
        dateObj[obj.date] = obj;
        if (i>0) {
            obj.lastDayObj = kLineJson[i-1];
        }
    }
...
function readKLineBaseSync(stockId, callback) {
    ...
    if (callback) callback(stockId, kLineJson);
    else return kLineJson;
...
exports.readKLineBaseInDateMap = readKLineBaseInDateMap;
