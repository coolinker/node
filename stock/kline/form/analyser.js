...
function traverseForWinning(method, klineJson, lossStop, winStop, daysStop, options) {
    ...
    for (var i = 50; i < len; i++) {
        ...
        if (date > endDate) break;
        if (options.passAll ||
            (bullklineforms[method](klineJson, i) || (options.union && unionResult(bullklineforms, options.union.split(","), klineJson, i))) && (!options.intersection || intersectionResult(bullklineforms, options.intersection.split(","), klineJson, i))
        ) {
...
function traverseForLosing(method, klineJson, lossStop, winStop, daysStop, options) {
    ...
    for (var i = 50; i < len; i++) {
        ...
        if (options.passAll ||
            (bearklineforms[method](klineJson, i) || (options.union && unionResult(bearklineforms, options.union.split(","), klineJson, i))) && (!options.intersection || intersectionResult(bearklineforms, options.intersection.split(","), klineJson, i))
        ) {
